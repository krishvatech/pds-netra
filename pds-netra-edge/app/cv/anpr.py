...
3) ✅ JSON-based whitelist/blacklist verification INSIDE anpr.py
   - VERIFIED / NOT_VERIFIED / BLACKLIST (no more UNKNOWN if JSON is used)
   - event_type: ANPR_PLATE_VERIFIED / ANPR_PLATE_ALERT / ANPR_PLATE_DETECTED
   - severity: info / warning / critical
4) ✅ Save crops to OUTSIDE folder using save_crops_dir or ENV path
"""
...
class PlateDetector:
    ...
    IMPORTANT:
    - If plate_class_names is empty/None, YOLO may output non-plate boxes
      (truck body, sign boards, text areas). That causes oversized crops.
    - We apply a geometry filter to keep only plate-like boxes.
    """
...
class OcrEngine:
    ...
    Strategy:
      1) det=True first
      2) fallback to det=False rec-only
    """
    ...
    def recognize(self, image_bgr: Any, min_conf: float = 0.3) -> Tuple[str, float]:
        if cv2 is None:
            return "", 0.0
        ...
        if hasattr(self.ocr, "predict"):
            ...
            try:
                ocr_results = self.ocr.ocr(image_rgb, det=True, cls=True)
                text, conf = _parse_paddle_ocr_detrec(ocr_results)
                if text and conf >= float(min_conf):
                    return text, float(conf)
            except Exception as exc:
            ...
            try:
                ...
                text2, conf2 = _parse_paddle_ocr_reconly(res)
...
class AnprProcessor:
    def __init__(
        ...
        # ✅ JSON whitelist/blacklist (outside project supported)
        # Env example: EDGE_ANPR_RULES_JSON="D:\PDS_RULES\approved_plates.json"
        ...
        if self.whitelist_plates or self.blacklist_plates:
            self.logger.info(
                "ANPR JSON rules loaded: whitelist=%d blacklist=%d path=%s",
                len(self.whitelist_plates), len(self.blacklist_plates), self.plate_rules_json
            )
        ...
        # ✅ save_crops_dir can be OUTSIDE folder (absolute path)
        # Env optional override: EDGE_ANPR_CROPS_DIR="D:\PDS_DEBUG\anpr_crop"
        ...
        # ✅ NEW: slight increase size around plate after shrink
    ...
    def process_frame(
        ...
        self.frame_index += 1
        if self.frame_index % self.ocr_every_n != 0:
            return results_out
        ...
        for (_, det_conf, bbox) in plates:
            ...
            try:
                ...
                # then add pad (your "increase size")
            ...
            if self.ocr_engine:
                try:
                    crop_pre = preprocess_plate_crop(crop)
                    plate_text_raw, ocr_conf = self.ocr_engine.recognize(crop_pre, min_conf=self.ocr_min_conf)

                    if not plate_text_raw:
                        crop_light = preprocess_plate_crop_light(crop)
                        plate_text_raw, ocr_conf = self.ocr_engine.recognize(crop_light, min_conf=self.ocr_min_conf)

                    if not plate_text_raw and cv2 is not None:
                        try:
                            gray = cv2.cvtColor(crop_pre, cv2.COLOR_BGR2GRAY)
                            thr = cv2.adaptiveThreshold(
                                gray, 255,
                                cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                cv2.THRESH_BINARY,
                            ...
                            plate_text_raw, ocr_conf = self.ocr_engine.recognize(thr_bgr, min_conf=self.ocr_min_conf)
                        except Exception:
                            pass
                except Exception as exc:
            ...
            event = EventModel(
                ...
                event_type=event_type,     # ✅ VERIFIED / ALERT / DETECTED
            ...
            self.logger.info(
                ...
                plate_text_display, zone_id,
                float(det_conf), float(ocr_conf), float(combined_conf),
