...
const mockAlerts: AlertItem[] = [
  {
    ...
    alert_type: "MOBILE_PHONE_USAGE",
    severity_final: "warning",
    status: "OPEN",
    start_time: new Date().toISOString(),
    summary: "Mobile phone usage detected",
    count_events: 1,
    key_meta: {
...
export default function PhonePage() {
        ...
        if (mounted)
          setError(
            friendlyErrorMessage(
              e,
              "Unable to load phone usage alerts. Please refresh or check your connection."
            )
          );
      }
    })();
  ...
  const filteredAlerts = useMemo(() => {
    ...
    return alerts.filter((a) => {
      const conf = a.key_meta?.phone_confidence;
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="space-y-2">
          <div className="hud-pill">Mobile phone usage</div>
          <div className="text-4xl font-semibold font-display tracking-tight text-slate-100 drop-shadow">
            Phone Usage Monitor
          </div>
          <div className="text-sm text-slate-300">Detects phone usage during active shift hours.</div>
        </div>
        <div className="intel-banner">Operational safety</div>
      </div>
      ...
      <Card className="hud-card">
        <CardHeader>
          <div className="text-lg font-semibold font-display">Phone usage alerts</div>
        </CardHeader>
        <CardContent>
              ...
              <tbody>
                {filteredAlerts.map((a) => (
                  <tr key={a.id} className="border-t border-white/10">
                    ...
                    <td className="py-2 pr-3">{a.key_meta?.phone_confidence ?? "-"}</td>
                    <td className="py-2 pr-3">
                ...
                {filteredAlerts.length === 0 && (
                  <tr>
                    <td colSpan={7} className="py-6 text-center text-slate-500">No phone usage alerts found.</td>
                  </tr>
                )}
              </tbody>
            </table>
