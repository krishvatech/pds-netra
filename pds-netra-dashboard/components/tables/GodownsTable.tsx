              ...
              <TD className="text-right">
                <div className="flex justify-end gap-2">
                  ...
                  <ConfirmDeletePopover
                      title={`Delete Godown: ${g.godown_id}`}
                      description={
                        <div className="space-y-2 text-xs text-slate-400">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              value={deleteTargetId === g.godown_id ? deleteConfirmText ?? '' : ''}
                              onChange={(e) => onDeleteConfirmTextChange?.(e.target.value)}
                              placeholder={g.godown_id}
                            />
                          </div>
                        </div>
                      }
                      confirmText="Delete"
                      confirmDisabled={(deleteConfirmText ?? '').trim() !== g.godown_id}
                      onConfirm={() => onDelete?.(g.godown_id)}
                      isBusy={deleteBusy}
                    >
                      <button
                        onClick={() => onDeleteInit?.(g.godown_id)}
